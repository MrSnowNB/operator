...
active_sos = {}  # Tracks active SOS events per sender: {sender_id: {trigger, target, timestamp}}
...
def on_receive(packet, interface):
    ...
    # === COMMAND: !safe (Cancel active SOS) ===
    if msg_lower == "!safe":
        with state_lock:
            sos_event = active_sos.pop(sender, None)

        if sos_event:
            timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
            original_trigger = sos_event['trigger']
            original_target = sos_event['target']
            ...
            # Route cancel to the same responder(s) who got the original dispatch
            if original_target:
                safe_send(cancel_msg, destinationId=original_target, channelIndex=incoming_chan)
                print(f"[SAFE] Cancel sent to {original_target}")
            elif all_responders:
                ...
                print("[SAFE] Cancel broadcast to channel")
            ...
            log_to_markdown(
                f"## SOS CANCELLED\n"
                f"**Original Trigger:** {original_trigger}\n"
                f"**Cancelled By:** {phone} ({sender})\n"
                f"**Time:** {timestamp}\n"
                f"**Responders Notified:** {original_target or 'ALL RESPONDERS'}\n"
                f"---"
            )
    ...
    if matched_trigger:
        ...
        time.sleep(2)  # Let LoRa TX buffer clear

        # 2. Safety follow-up — tell citizen how to cancel
        ...
        # 4. Route to specific responder OR broadcast
        ...
        # 5. Track this SOS so !safe can reference it
        with state_lock:
            active_sos[sender] = {
        ...
        # 7. Queue context for AI triage if present
        if context:
            message_queue.put({
                'sender': sender,
                'message': context,
